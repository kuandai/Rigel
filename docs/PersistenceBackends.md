...
segments, platform-reserved names, and non-canonical spellings are rejected.
Namespaced IDs are stored as two components (`rigel/default`), so the encoding
is injective over accepted IDs:
