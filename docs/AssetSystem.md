...
Model faces without valid cube-style AO metadata use conservative unoccluded
vertex values, and `none` represents explicit empty geometry. In an interactive
build block resources are generated by `scripts/rigel_assets.py`; normal tests
use small synthetic definitions and do not depend on Cosmic Reach content.
