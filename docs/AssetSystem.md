...
the complete reusable-model and block-definition set before publishing either
registry. Blocks bind their own texture paths to immutable model texture slots;
the built-in full cube remains the specialized default, and `none` represents
explicit empty geometry. In an interactive build block resources are generated
by `scripts/rigel_assets.py`; normal tests use small synthetic definitions and
do not depend on Cosmic Reach content.
