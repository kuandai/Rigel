...
TEST_CASE(ChunkStreamer_ReplacementRejectsMeshBuiltWithPriorNeighbor) {
    ChunkManager manager;
    BlockRegistry registry;
    WorldMeshStore meshStore;
    auto generator = makeGenerator(registry);
    BlockID solid = registerTestBlock(registry, "rigel:replacement_boundary_solid");
    const ChunkCoord survivingCoord{0, 0, 0};
    const ChunkCoord replacedCoord{1, 0, 0};
    ...
    Chunk& replaced = manager.getOrCreateChunk(replacedCoord);
    replaced.setBlock(0, 1, 1, BlockState{solid}, registry);
    replaced.setWorldGenVersion(generator->config().world.version);
    replaced.setLoadedFromDisk(true);
    ...
    Chunk replacement(replacedCoord);
    manager.loadChunk(replacedCoord, replacement.serialize());
    CHECK_EQ(surviving.meshRevision(), queuedRevision + 1);

    streamer.update(survivingCoord.toWorldCenter());
    CHECK_EQ(streamer.workMetrics().meshJobsStarted, static_cast<uint64_t>(1));
    CHECK_EQ(streamer.workMetrics().meshInvalidations, static_cast<uint64_t>(1));
    CHECK_EQ(streamer.workMetrics().meshRequestsCoalesced, static_cast<uint64_t>(1));
    ...
    CHECK_EQ(metrics.meshJobsAccepted, static_cast<uint64_t>(1));
    CHECK_EQ(metrics.meshJobsRejectedStale, static_cast<uint64_t>(1));
    CHECK_EQ(metrics.meshInvalidations, static_cast<uint64_t>(1));
    CHECK_EQ(metrics.meshRequestsCoalesced, static_cast<uint64_t>(1));
