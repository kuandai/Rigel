...
`solid`, collision, `emits_light`, and `light_attenuation` remain properties of
the block registration emitted from source fields. The current entity resolver
still applies legacy whole-cell `solid` collision, and edit raycasts hit occupied
cells rather than model faces; neither path derives bounds from visual geometry.
